...
import { createEmployer } from "@/backend/services/employerSvc";
import { registerEmployerSchema } from "@/backend/api/schemas";
import { RegisterEmployerSchema } from "@/backend/api/types";
...
export default function Page() {
  const form = useForm<RegisterEmployerSchema>({
  ...
  function onSubmit(values: RegisterEmployerSchema) {
    createEmployer(values).then((success) =>
      success
  ...
  return (
    <main className="flex flex-row min-h-screen">
      <section className="w-[808px] max-w-[50%] bg-gradient-to-t from-primary-lighter to-primary-background flex flex-col items-center text-center pt-44 pb-20 shadow-md">
        <h1 className="text-4xl font-bold w-[512px]">
      ...
      <section className="flex flex-col items-center min-w-[50%] justify-center space-y-[56px]">
        <h1 className="text-2xl font-normal">Join as employer</h1>
        <Form {...form}>
          <form
